...
async function processQueueItem(item, workerId, retryCount = 0) {
    ...
    try {
        ...
        if (retryCount < WORKER_CONFIG.MAX_RETRIES) {
            const isRetryableError = error instanceof Error && (error.message.includes('timeout') ||
...
async function worker(workerId) {
    ...
    while (true) {
        try {
            ...
            const item = await (0, redis_1.getNextQueueItem)();
            if (item) {
            ...
            const connectionValid = await (0, redis_1.checkRedisConnection)();
            if (!connectionValid) {
...
async function main() {
    ...
    // Iniciar workers
    const workers = Array.from({ length: WORKER_CONFIG.CONCURRENT_WORKERS }, (_, i) => worker(i + 1));
